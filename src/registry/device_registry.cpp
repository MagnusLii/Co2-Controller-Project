...
DeviceRegistry::DeviceRegistry() {
    xTaskCreate(initialize_task, "initialize_registry", 512, this,
                TASK_PRIORITY_ABSOLUTE, nullptr);
}
...
void DeviceRegistry::initialize() {
    auto co2 = std::make_shared<ModbusReadHandler>(
        mbctrl, MB_DEVADDR_CO2, MB_REGADDR_CO2, 2, true, ReadingType::CO2,
        "CO2 Level");
    auto temp = std::make_shared<ModbusReadHandler>(
        mbctrl, MB_DEVADDR_TEMP, MB_REGADDR_TEMP, 2, true,
        ReadingType::TEMPERATURE, "Temperature");
    auto hum = std::make_shared<ModbusReadHandler>(
        mbctrl, MB_DEVADDR_REL_HUM, MB_REGADDR_REL_HUM, 2, true,
        ReadingType::REL_HUMIDITY, "Humidity");
    auto fan_counter = std::make_shared<ModbusReadHandler>(
        mbctrl, MB_DEVADDR_FAN, MB_REGADDR_FAN_COUNTER, 1, false,
        ReadingType::FAN_COUNTER, "Fan Counter");
    auto w_fan_speed = std::make_shared<ModbusWriteHandler>(
        mbctrl, MB_DEVADDR_FAN, MB_REGADDR_FAN_SPEED, 1, WriteType::FAN_SPEED,
        "Fan Speed Control");
    auto pressure = std::make_shared<I2CHandler>(
        i2c, I2C_DEVADDR_PRESSURE, ReadingType::PRESSURE, "Pressure");

    float target_from_eeprom = 400.0;
    fanctrl = std::make_shared<FanController>(
        w_fan_speed->get_write_queue_handle(), target_from_eeprom);
...
void DeviceRegistry::subscribe_to_handler(const ReadingType type,
                                          QueueHandle_t receiver) {
    if (read_handlers.find(type) != read_handlers.end()) {
        read_handlers[type]->add_subscriber(receiver);
        std::cout << "Subscriber added to " << read_handlers[type]->get_name()
                  << std::endl;
    } else {
...
QueueHandle_t DeviceRegistry::get_write_queue_handle(const WriteType type) {
    if (write_handlers.find(type) != write_handlers.end()) {
        std::cout << "Subscriber given handle to "
                  << write_handlers[type]->get_name() << std::endl;
        switch (type) {
...
void DeviceRegistry::add_register_handler(
    std::shared_ptr<ReadRegisterHandler> handler, const ReadingType type) {
...
// exists
void DeviceRegistry::add_register_handler(
    std::shared_ptr<WriteRegisterHandler> handler, const WriteType type) {
...
TestSubscriber::TestSubscriber() {
    ...
    xTaskCreate(receive_task, "Receive Task", 256, this, TASK_PRIORITY_MEDIUM,
                nullptr);
}

TestSubscriber::TestSubscriber(const std::string &name) : name(name) {
    receiver = xQueueCreate(10, sizeof(Reading));
    xTaskCreate(receive_task, name.c_str(), 256, this, TASK_PRIORITY_MEDIUM,
                nullptr);
}
...
TestWriter::TestWriter(const std::string &name, QueueHandle_t handle) {
    ...
    xTaskCreate(send_task, name.c_str(), 256, this, TASK_PRIORITY_MEDIUM,
                nullptr);
}
